...
class DamagedCryosleeperInteraction : RATInteractionPlugin() {
    ...
    override fun init() {
        ...
        if (!interactionTarget.hasTag("rat_defenders_defeated")) {
            createOption("Continue") {
                ...
                textPanel.addPara("An automated drone is firing upon any debris in its view. It appears that what sealed the fate of this ship is a defect in a protocol of its voyage's defender.")

                textPanel.addPara("Any attempt to approach will most likely result in it choosing us as the next piece of \"debris\".")
    ...
    override fun defeatedDefenders() {
        ...
        textPanel.addPara("Once the threat has been disabled the crew makes its way towards the cryosleeper's interior. As expected the cryopods' life-support has long gone offline, not a single one showing a sign of life.")


        textPanel.addPara("This cryosleeper seems rather well supplied with equipment for the construction of an initial colony. It is especially well prepared for the habitation of a planet with few resources.")
